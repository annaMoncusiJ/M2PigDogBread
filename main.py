...
def load_and_process_image(url_img):
    if url_img.endswith(".png"):
        ...
        img_tensor = transform(img_pil)
        img_tensor = img_tensor.reshape(1, -1)  # Redimensionar para que sea compatible con el clasificador SVM
        return img_tensor
    else:
...
print(f'La imagen de entrada pertenece a la clase: {predicted_class[0]}')
